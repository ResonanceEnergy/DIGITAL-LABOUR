...
Supported directive types:
  ...
  - ncc.relay                   — publish event to NCC relay
"""
...
PROJECT_ROOT = NCC_ROOT.parent
ADAPTERS_DIR = NCC_ROOT / "adapters"
DECISION_LOG = PROJECT_ROOT / "data" / "ncc_decisions.jsonl"
...
def _ncc_relay(d: dict) -> dict:
    try:
        ...
        return {"executed": True, "action": "ncc.relay", "published": ok}
    except Exception as e:
        return {"executed": False, "error": str(e)}
...
def dispatch(directive: dict) -> dict:
    ...
    _log.info("Orchestrator dispatching: %s", dtype)
...
def _log_decision(directive: dict, result: dict):
    """Append directive + result to NCC decision audit trail."""
...
def pending_decisions(limit: int = 20) -> list[dict]:
    """Read recent NCC decisions from audit trail."""
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="NCC Orchestrator")
