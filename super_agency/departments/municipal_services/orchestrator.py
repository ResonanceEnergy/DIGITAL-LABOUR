"""Municipal Services Division Orchestrator.

Autonomous intake → dispatch → QA → delivery pipeline for municipal
documentation: meeting minutes, public notices, ordinances, grant
applications, budget summaries, and annual reports.

Targets small/mid-size municipalities with limited staff.
...
logger = logging.getLogger("division.mun_svc")
...
def _load_config() -> dict:
    try:
        ...
        logger.warning("Failed to load municipal_services config: %s", e)
        return {}
...
DOC_TYPES = {
    "meeting_minutes": {"agent": "data_reporter", "template": "minutes"},
    "public_notice": {"agent": "compliance_docs", "template": "public_notice"},
    "ordinance": {"agent": "compliance_docs", "template": "ordinance"},
    "resolution": {"agent": "compliance_docs", "template": "resolution"},
    "grant_application": {"agent": "grant_writer", "template": "municipal_grant"},
    "budget_summary": {"agent": "data_reporter", "template": "budget"},
    "annual_report": {"agent": "data_reporter", "template": "annual_report"},
    "rfp_document": {"agent": "proposal_writer", "template": "municipal_rfp"},
    "agenda": {"agent": "data_reporter", "template": "agenda"},
    "staff_report": {"agent": "data_reporter", "template": "staff_report"},
}


class DivisionTracker:
    def __init__(self, max_daily: int = 35, max_cost: float = 0.30, max_tokens: int = 500000):
        ...
        with self._lock:
            if success:
                ...
                if self._consecutive >= self.threshold:
                    ...
                    logger.warning("[MUN-SVC] Circuit breaker tripped — cooldown %d min", self.cooldown_minutes)
...
class MunicipalDivision:
    """Self-contained autonomous division for municipal document production."""

    def __init__(self):
        self.tracker = DivisionTracker(
            max_daily=AUTONOMY.get("max_daily_tasks", 35),
            max_cost=AUTONOMY.get("max_cost_per_task_usd", 0.30),
    ...
    async def process(self, request: dict) -> dict:
        ...
        doc_type = request.get("doc_type", "meeting_minutes")
        start = time.time()
        ...
        logger.info("[MUN-SVC][%s] Intake accepted — doc_type=%s, agent=%s, tier=%s", task_id, doc_type, route["agent"], tier)
        ...
        while attempt <= self.max_retries:
            ...
            try:
                ...
                logger.warning("[MUN-SVC][%s] Attempt %d failed: %s", task_id, attempt, exc)
                if attempt > self.max_retries:
                    elapsed = time.time() - start
                    self.breaker.record(False)
                    self.tracker.record_task(0.0, 0, False, 0, tier, 0)
                    return {"task_id": task_id, "status": "failed", "reason": "All attempts failed", "elapsed_seconds": round(elapsed, 2)}
    ...
    async def _run_agent(self, agent_name: str, request: dict, token_budget: int, task_id: str, attempt: int) -> dict:
        ...
        try:
            ...
            logger.error("[MUN-SVC][%s] Agent %s not found: %s", task_id, agent_name, exc)
            raise
        except Exception as exc:
            logger.error("[MUN-SVC][%s] Agent %s error (attempt %d): %s", task_id, agent_name, attempt, exc)
            raise

    def health_check(self) -> dict:
        breaker_open, breaker_reason = self.breaker.is_open()
        return {
            "division": "municipal_services", "division_code": "MUN-SVC",
    ...
    def reset_breaker(self):
        ...
        logger.info("[MUN-SVC] Circuit breaker manually reset")
