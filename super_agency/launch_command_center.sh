...
echo "🚀 Bit Rage Systems Distributed Command Center"
echo "=========================================="
echo ""
...
show_status() {
    echo ""
    log_success "Bit Rage Systems Command Center Status"
    echo "====================================="
    ...
    if command -v aws &> /dev/null && aws sts get-caller-identity &> /dev/null; then
        ...
        # Check EC2 instances
        INSTANCE_COUNT=$(aws ec2 describe-instances --filters "Name=tag:Project,Values=Bit Rage Systems" "Name=instance-state-name,Values=running" --query 'length(Reservations[*].Instances[*])' --output text 2>/dev/null || echo "0")
...
show_menu() {
    echo ""
    echo "Bit Rage Systems Command Center Menu"
    echo "================================="
