#!/usr/bin/env python3
"""
Bit Rage Systems Advanced Monitoring Dashboard
...
class AdvancedMonitoringDashboard:
    ...
    def display_dashboard(self):
        ...
        print("🚀 Bit Rage Systems Advanced Monitoring Dashboard")
        print("=" * 60)
        ...
        print(
            f"   Memory Usage: {sys_metrics['memory']['usage_percent']:.1f}%")
        ...
        print("\n🏆 TOP PROJECTS BY TIER:")
        executive_projects = [
            p for p in self.projects if p.get('tier') == 'EXECUTIVE']
        large_projects = [p for p in self.projects if p.get('tier') == 'L'][:3]
        medium_projects = [
            p for p in self.projects if p.get('tier') == 'M'][:3]

        if executive_projects:
            print(f"   EXECUTIVE: {executive_projects[0]['name']}")
        if large_projects:
            print(
                f"   LARGE: {', '.join([p['name'] for p in large_projects])}")
        if medium_projects:
            print(
                f"   MEDIUM: {', '.join([p['name'] for p in medium_projects])}")
        ...
        try:
            ...
            print("\n🎛️ CPU & TASK MANAGER:")
            print(
                f"   Status: {'🟢 Active' if ctm_status['overall_status'] == 'running' else '🔴 Inactive'}")
            tm = ctm_status['task_manager']
            print(
                f"   Tasks - Running: {tm['running_tasks']}, Queued: {tm['queued_tasks']}, Completed: {tm['completed_tasks']}")
            cpu_reg = ctm_status['cpu_regulator']
            print(
                f"   CPU Regulation - Current: {cpu_reg['current_cpu']:.1f}%, Throttled: {cpu_reg['throttled_processes']}")
        except ImportError:
            print("\n🎛️ CPU & TASK MANAGER:")
        ...
        if summary['high_risk_projects'] > 0:
            print(
                f"   ⚠️  {summary['high_risk_projects']} high-risk projects need attention")
