...
<title>Digital Labour — Free Demo</title>
<style>
...
async def run_demo(req: DemoRequest):
    ...
    try:
        ...
        output = call_llm(
            system_prompt=f"You are a professional {service['label']} writer for DIGITAL LABOUR agency. "
        ...
        return JSONResponse({"error": f"Demo generation failed: {str(e)[:100]}"}, status_code=500)
    ...
    try:
        ...
        send_email(
            to=req.email,
            subject=f"Your Free {service['label']} — Digital Labour",
...
<p>Get unlimited access: <a href="https://digital-labour.com/signup">Sign up now</a></p>
<p>Questions? Reply to this email — a human will respond.</p>
<p style="color:#888;font-size:12px">Digital Labour — AI Workforce on Demand<br>
...
""",
    ...
    return JSONResponse({
        ...
        "output": output,
        "cta": "Impressed? Get unlimited access at https://digital-labour.com/signup",
    })
