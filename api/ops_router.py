"""Bit Rage Operations Router — Division ops, health, goals, dispatch.

Exposes the BRL Ops Commander functions as REST endpoints so the
workstation dashboard can trigger and display them.

Endpoints:
    GET  /api/v1/ops/status          — Full ops status report
    GET  /api/v1/ops/health          — Division health check
    POST /api/v1/ops/daily-push      — Trigger daily ops push
    POST /api/v1/ops/weekly-goals    — Force weekly goal generation
    GET  /api/v1/ops/weekly-goals    — Read latest weekly goals
    GET  /api/v1/ops/divisions       — Division definitions + live state
    GET  /api/v1/ops/escalations     — Recent escalation log
    POST /api/v1/ops/dispatch/:div   — Fire warm-up task to a division
    POST /api/v1/ops/internal-ops    — Trigger internal operations engine
    GET  /api/v1/ops/output-brief    — Intelligence brief from output store
"""
...
router = APIRouter(prefix="/api/v1/ops", tags=["Bit Rage Operations"])

PROJECT_ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = PROJECT_ROOT / "data"
WEEKLY_GOALS_DIR = DATA_DIR / "weekly_goals"
OPS_STATE_FILE = DATA_DIR / "ncl_ops_state.json"  # backward compat filename
...
def _get_divisions() -> dict:
    ...
    try:
        from dispatcher.ops_commander import DIVISIONS
        return DIVISIONS
    except ImportError:
        return {}
...
async def ops_status():
    ...
    return {
        "commander": "Bit Rage Operations Commander",
...
async def ops_health():
    ...
    try:
        from dispatcher.ops_commander import check_division_health
...
async def ops_daily_push():
    """Trigger daily operations push."""
    try:
...
async def ops_generate_weekly_goals():
    """Force weekly goal generation for all divisions."""
    try:
        from dispatcher.ops_commander import generate_weekly_goals
...
async def ops_get_weekly_goals():
...
async def ops_dispatch_division(division: str, req: DispatchRequest):
    ...
    try:
        from dispatcher.ops_commander import SERVICE_TO_TASKTYPE
...
async def ops_internal_ops(mode: str = "daily"):
