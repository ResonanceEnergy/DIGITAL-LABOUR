# SUPER AGENCY → BIT RAGE SYSTEMS — Full Integration Roadmap

**Date:** 2026-03-15
**Objective:** Complete and absolute merger of DIGITAL-LABOUR into BIT RAGE SYSTEMS.
...
| BIT RAGE SYSTEMS Core files (excl repos/venv/git) | 1,678 |
| DL super_agency/ current files | 826 |
| Missing from DL | ~853 files |
| "BIT RAGE SYSTEMS" refs in SA | ~1,018 |
| "BIT RAGE SYSTEMS/BIT RAGE SYSTEMS" refs in DL root | ~200+ |
| Rename target | BIT RAGE SYSTEMS → BIT RAGE SYSTEMS |
...
## PHASE 2: RENAME — BIT RAGE SYSTEMS → BIT RAGE SYSTEMS
**Goal:** All display names, codenames, docstrings reference "BIT RAGE SYSTEMS"

### In super_agency/:
- `__init__.py`: `__codename__` → "BIT RAGE SYSTEMS"
...
### In DL root files:
- `matrix_boot.py`: "BIT RAGE SYSTEMS MATRIX" → "BIT RAGE SYSTEMS MATRIX"
- `api/matrix_monitor.py`: "BIT RAGE SYSTEMS MATRIX" → "BIT RAGE SYSTEMS MATRIX"
- `api/matrix_dashboard.html`: "BIT RAGE SYSTEMS MATRIX" → "BIT RAGE SYSTEMS MATRIX"

### PRESERVE (do NOT rename):
- `site/` — "BIT RAGE SYSTEMS" is the customer-facing BRAND name
- `campaign/` — marketing copy references BIT RAGE SYSTEMS brand
- `income/freelance_listings.py` — platform profile content
- `automation/` — outreach templates (BIT RAGE SYSTEMS brand)
...
**Goal:** All hardcoded BIT RAGE SYSTEMS paths point to DL's super_agency/
...
- Update README.md: "ARCHIVED — Merged into BIT RAGE SYSTEMS"
