...
call_llm = make_bridge("proposal_writer")
...
def writer_agent(
    ...
    return safe_validate(WriterOutput, data, agent_name="proposal_writer.writer")
...
def qa_agent(proposal: WriterOutput, provider: str = "openai") -> QAResult:
    ...
    raw = call_llm(system_prompt=system, user_prompt=user_msg,
                   provider=provider, json_mode=True)
    data = json.loads(raw, strict=False)
    return safe_validate(QAResult, data, agent_name="proposal_writer.qa")
