"""BITRAGE SECURITY AUDIT — Automated vulnerability scanner.
...
def fetch(url, method="GET", data=None, headers=None, timeout=10):
    ...
    hdrs = {"User-Agent": "BitRage-SecurityAudit/1.0"}
    if headers:
        hdrs.update(headers)
...
def audit_website():
    ...
    api_urls = [u for u in urls_found if "railway" in u or "bitrage-labour-api" in u]
    if api_urls:
...
def generate_report():
    print("\n" + "═" * 60)
    print("  BITRAGE SECURITY AUDIT — SUMMARY")
